...
public class AutoEXPPlus extends Module {
    ...
    private boolean isUsingElytraFly() {
        ...
        return module != null && module.isActive() && mc.player != null && mc.player.isFallFlying();
    }
    ...
    private void elytraFlyLanding() {
        ...
        switch (elytraPhase) {
            ...
            case HOVER -> {
                if (isElytraHovering() || mc.player.onGround() || mc.player.isInWater()) {
                    if (++landedTicks >= 5) {
                        canRestartFlight = true;
                        phase = Phase.REPAIR;
                    }
    ...
    private void startElytraFlyLanding() {
        ...
        oldElytraKeyShift = mc.options.keyShift.isDown();
        oldElytraKeyUp = mc.options.keyUp.isDown();
    ...
    private boolean platformBuiltBelow() {
        ...
        return state.isSolid() && mc.player.position().y > below.getY() + 1.05;
    }
    ...
    private boolean isElytraHovering() {
        ...
        return Math.abs(mc.player.position().y - hoverY) < 0.5;
    }
    ...
    private void restoreElytraFly() {
        ...
        mc.options.keyShift.setDown(oldElytraKeyShift);
        mc.options.keyUp.setDown(oldElytraKeyUp);
    ...
    private boolean isLanded() {
        if (mc.player == null || mc.level == null) return false;
        if (mc.player.onGround() || mc.player.isInWater()) return true;
